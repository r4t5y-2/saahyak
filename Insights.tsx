...
export default function Insights() {
  ...
  const mirrorEvents = useMutation(api.sahaayak.pushEvents);
  const mirrorRepairs = useMutation(api.sahaayak.pushRepairs);
  const mirrorSentences = useMutation(api.sahaayak.pushSentences);
  const clearMirror = useMutation(api.sahaayak.clearMirror);

  const mirror = useQuery(api.sahaayak.getEvents);
  const mirrorRepairsCount = useQuery(api.sahaayak.getRepairs)?.length ?? 0;
